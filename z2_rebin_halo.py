...
for i_halo in [4, 10, 20, 45, 46, 48, 59, 69, 71, 73, 76, 85, 87, 91, 100, 102, 110]:#np.arange(0,np.sum(mass_filter_0)):
    ...
    fig3, axes3 = plt.subplots(nrows=1, ncols=4, figsize=(6,10),gridspec_kw=gridspec)
    
    #axes2[0].scatter(subhalo_posx[cond_all],wl_halo[cond_all], marker='*', s=2, c='red')
    c2 = axes3[0].pcolormesh(x_grid,wl_rebin , map_plot.T, cmap='Greys', vmin=0., vmax=1.)
    ...
    ax3.set_yticks(vel_plot)
    ax3.set_ylabel(r'$\Delta v$ km/s',fontsize=14)
    
    N_y_ticks = np.ceil((vel_plot[-1]-vel_plot[0])/200)+2
    ...
    ax4.yaxis.tick_left()
    ax4.set_yticks(np.int32(dl_arr_plot))
    ...
    ax32.set_yticks(vel_plot)
    ax32.set_ylabel(r'$\Delta v$ km/s',fontsize=14)
    ...
    axes3[2].set_ylim(np.max([wl_rebin[0],wl_halo_0-2*halo_0_dwl]),np.min([wl_rebin[-1],wl_halo_0+2*halo_0_dwl]))
    #axes3[0].set_ylim(d_arr_z[z_ind_plot][0],d_arr_z[z_ind_plot][-1])
    ...
    ax42.set_yticks(np.int32(dl_arr_plot))
