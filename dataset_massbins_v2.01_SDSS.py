...
for ibin in np.arange(0,len(mass_bins)-1):
    ...
    for j, (pos, radius) in enumerate(zip(subhalo_positions, subhalo_radii)):
        ...
        if j<=3:
            print(mass_bins[ibin],'-',mass_bins[ibin+1],len(subhalo_positions),j, los_indices_j,(pos, radius))
        ...
        for k in los_indices_j:
            ...
            LOS_MASK[k, wl_indices_j] = ibin+1
...
# Flatten the arrays to ensure they are 2D
test_flux = test_flux.reshape(-1, test_flux.shape[-1])
test_mask = test_mask.reshape(-1, test_mask.shape[-1])

# Generate a random permutation of indices
indices = np.random.permutation(test_flux.shape[0])

# Shuffle the arrays using the random indices
test_flux = test_flux[indices]
test_mask = test_mask[indices]


version = f'sdss_test_{N_los_halo_bin:d}_{N_los_NO_halo:d}'
# Save flux_subset and LOS_MASK to an HDF5 file
with h5py.File('ML_SDSS_training_z'+str(z_0)[0:3]+version+'.hdf5', 'w') as hf:
...
for ibin in np.arange(0,len(los_halo_bins)):
    ...
    version = 'SDSS_test'
    version_tag = version + f'_bin{ibin:d}'
    print(version_tag)
    ...
    #                fontsize=10, color='black', ha='left', va='top')
        ...
        ax2.set_ylim(-0.5, len(los_halo_bins)+1)
        ax.set_xlim(ray_z[0], ray_z[-1])
    
        plt.savefig('ML_spec' + str(x) + '_plot'+version_tag+'.pdf')
        plt.close()
    
    
    fitpdf = sorted([os.path.basename(x) for x in glob.glob('ML_spec*'+version_tag+'*.pdf')])
    merger = PdfMerger()
    for pdf in fitpdf:
        merger.append(pdf)
        os.remove(pdf)
    merger.write(f'plot_SPEC_ML_N{plotN:d}_'+version_tag+'.pdf')
    merger.close()
